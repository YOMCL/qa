 ...
 */
test.describe('PAG — Pagos y documentos tributarios (B2B)', () => {

  test('PAG-04: Historial de pagos carga correctamente', async ({ authedPage: page }) => {
  ...
  test('PAG-16: Montos en historial no muestran valores negativos confusos', async ({ authedPage: page }) => {
    // Navegar a pagos
    await page.goto('/payments');
    await page.waitForLoadState('networkidle');
    ...
    await page.screenshot({ path: 'test-results/payments-no-negatives.png', fullPage: true });
  ...
  test('PAG-17: Documentos tributarios accesibles', async ({ authedPage: page }) => {
    // Navegar a documentos tributarios / facturas
    const docLinks = [
      page.getByRole('link', { name: /[Ff]actura/ }),
      page.getByRole('link', { name: /[Dd]ocumento/ }),
      page.getByRole('link', { name: /[Tt]ributari/ }),
    ];
    ...
    await page.waitForLoadState('networkidle');
    await page.screenshot({ path: 'test-results/tax-documents.png', fullPage: true });
